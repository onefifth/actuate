...
class Actuate {
	...
	/**
	 * Checks if a target is being tweened
	 * @param	target	The object to check
	 * @return		Whether or not the target is tweening.
	 */
	public static function isTweening<T> (target:T):Bool {
		#if neko

		if (Reflect.isFunction (target)) {
			return methodLibraries.exists (target));
		}

		#end

		return targetLibraries.exists (target);
	}
